...
//		List<CuentaEstadoResultados> cuentas = estadoResultadosService.obtenerCuentasEstadoResultados();
		...
		List<CuentaEstadoResultados> cuentas = estadoResultadosService
		...
		for (Map.Entry<String, List<String>> grupo : gruposCuentas.entrySet()) {
			String listaCuentas = grupo.getValue().stream().map(c -> "'" + c + "'").collect(Collectors.joining(", "));
			...
			String query = "SELECT '" + grupo.getKey() + "' AS Grupo, "
					...
					+ "JOIN prod_dwh_consulta.PUC P ON P.Puc_ID = ef.Puc_ID " + "WHERE T.Fecha = '" + fecha + "' "
					+ "AND E.Tipo_Entidad = 23 " + "AND E.Codigo_Entidad = " + codigoEntidad + " "
					+ "AND ef.Tipo_Informe = 0 " + "AND P.codigo IN (" + listaCuentas + ")";
...
//		List<String> codigosPUC = estadoResultadosService
